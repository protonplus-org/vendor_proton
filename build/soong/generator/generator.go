...
func init() {
	android.RegisterModuleType("proton_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = protonExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = protonExpandVariables(ctx, dep_file)
	...
	exCmd := protonExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "proton.sbox.textproto"
	desc := "generate"
	name := "generator"
